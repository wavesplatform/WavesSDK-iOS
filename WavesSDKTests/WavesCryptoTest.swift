...
private enum Constants {
    static let mockSeed: String = "pattern voyage train body member denial nominee face useful engage fortune spatial inmate bright below";
    ...
    static let chainId = "W"
    
    static let randomBytes: [UInt8] = [0, 3, 4, 5]
}

final class WavesCryptoTest: XCTestCase {
    ...
    func testAddressValid() {
        ...
        let address = wavesCrypto.verifyAddress(address: Constants.mockSeedAddress, chainId: Constants.chainId, publicKey: Constants.mockSeedPublicKey)
    ...
    func testSignValid() {
        ...
        let signature = wavesCrypto.signBytes(bytes: Constants.randomBytes, privateKey: wavesCrypto.privateKey(seed: Constants.mockSeed)!)
        
        let result = wavesCrypto.verifySignature(publicKey: wavesCrypto.publicKey(seed: Constants.mockSeed)!, bytes: Constants.randomBytes, signature: signature!)
